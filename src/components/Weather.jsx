...
function Weather() {
    ...
    const handleButtonTp = () => {
        navigate(`/cities/${record.name}`);
    };

    return (
        <div className="bg-main-bg h-screen flex flex-col justify-center items-center">
            <div className="w-full overflow-hidden hidden sm:block py-2 relative">
                <div className="flex justify-around items-center whitespace-nowrap animate-marquee">
                    {citiesTop.map((item) => {
                        return (
                            <button
                                key={item}
                ...
                <div className="border-2 bg-white/30 backdrop-blur-md p-6 rounded-2xl">
                    ...
                    {record ? (
                        <div className="text-center mt-6 transition-all">
                            <p className="text-2xl font-semibold">{record.name}</p>
                            {record.weather
                                ...
                                : null}
                            <p className="text-3xl font-bold">{record.main.temp} °C</p>
                            <p className="text-lg text-gray-500">{record.wind.speed} km/h</p>
                            <button
